...
$qRecent = "SELECT sale_date, total_amount FROM sales_record ORDER BY sale_date DESC LIMIT 10";
...
for ($i = 6; $i >= 0; $i--) {
    ...
    $queryTrend = "SELECT IFNULL(SUM(total_amount),0) as dayTotal FROM sales_record WHERE DATE(sale_date) = '$date'";
    $resultTrend = $conn->query($queryTrend);
    if ($resultTrend) {
        $rowTrend = $resultTrend->fetch_assoc();
        $trendData[] = (float)$rowTrend['dayTotal'];
    } else {
        $trendData[] = 0;
    }
}
...
// B. Customer Loyalty Report: Groups sales by customer.
...
<div class="dropdown-container">
  ...
  <div class="content">
      ...
      <div class="report-section">
          ...
          <!-- B. Customer Loyalty Report -->
          <div class="card mb-3">
              <div class="card-header bg-dark text-white">Customer Loyalty Report</div>
              <div class="card-body">
                  <div class="table-responsive">
  ...
  <script>
    ...
    document.querySelectorAll('.dropdown-btn').forEach(btn => {
      ...
      // Initialize DataTables for Recent Sales, Ledger, Product Performance, and Customer Loyalty tables
